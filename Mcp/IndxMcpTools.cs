...
namespace IndxServer.Mcp
{
        ...
        DatasetMetadataStore metadata)
    {
        ...
        [McpServerTool(Name = "diagnose_search", UseStructuredContent = false, ReadOnly = true)]
        [System.ComponentModel.Description("Explain why a search returns nothing. Checks the dataset state, whether any field is " +
                     "searchable, the query length, and the engine's own error message, then runs the query and reports what it " +
                     "found. Use this instead of guessing when a search is unexpectedly empty.")]
        public async Task<JsonObject> DiagnoseSearch(
            [System.ComponentModel.Description("Team name that owns the dataset.")] string team,
            [System.ComponentModel.Description("Dataset name.")] string dataset,
            [System.ComponentModel.Description("The query text that returned nothing.")] string query)
        {
            var ownerKey = await ResolveOwnerKey(team, dataset, ApiKeyLevel.Read);
            ...
            {
                findings.Add($"The dataset is {status.SystemState}, not Ready, so no search can match. Run index and poll get_status.");
                return new JsonObject { ["query"] = query, ["hitCount"] = 0, ["findings"] = findings };
            }
            if (status.DocumentCount == 0)
                findings.Add("The dataset holds no documents. Load data before searching.");

            var engine = IndxServerInternalApi.Manager.ResolveEngine(dataset, ownerKey)
                ?? throw new McpToolException($"Dataset '{dataset}' not found.");
            var cfg = engine.GetFieldConfiguration();
            if (cfg.All(f => f.Searchable != true))
                findings.Add("No field is searchable. Set Searchable on at least one text field and re-index.");
            ...
            // non-searchable field is the documented case, and its message lives only here.
            var after = IndxServerInternalApi.Manager.GetState(dataset, ownerKey);
            if (after != null)
            {
                if (after.TooLongSearchText)
                    findings.Add("The query text is longer than the configured limit, so the search was refused rather than truncated. Shorten it.");
                if (!string.IsNullOrWhiteSpace(after.ErrorMessage))
